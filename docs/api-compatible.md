...
## DeepSeek local debugging

For local debugging, seed data includes `deepseek-chat`, routed to the `deepseek` provider. Use your gateway API key as the client key and keep the SDK `base_url` pointed at this gateway, for example `http://localhost:3000/v1`.
